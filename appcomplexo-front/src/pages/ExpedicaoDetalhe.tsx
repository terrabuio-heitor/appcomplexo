  ...
  useEffect(() => {
    const carregarEventos = async () => {
      ...
      if (Array.isArray(resultado)) {
        ...
        const filtrados = resultado.filter(ev => {
          const idVinculado = ev.exID || (ev as any).expedicao_id
          return Number(idVinculado) === Number(id)
        })
                ...
                <div className="space-y-4">
                  {eventos.map(ev => (
                    <div key={ev.id} className="border-b border-slate-100 pb-3 last:border-0">
                      ...
                      <p className="text-xs text-slate-400">📅 {ev.data}</p>
                    </div>
                  ))}
                </div>
              ) : (
